...
all_sectors = ['Information Technology', 'Communication Services','Financials',
               'Health Care', 'Real Estate', 'Consumer Discretionary', 'Energy',
...
companies = st.sidebar.number_input("Number of Companies", min_value=1, max_value=500, value=100)

#Sector Selections
display_sector_select = st.sidebar.checkbox('Select a Sector')
display_company_select = st.sidebar.checkbox('Select a Company')
company_compare = st.sidebar.checkbox('Compare Companies')
select_Sector = 0
select_Company = 0
compare_company_select = 0
final_comp_ticker_list = []
if display_company_select or company_compare:
    data_file = f'input/RAY_2024-06-28.xlsx'
    df_company = pd.read_excel(data_file)
    df_company.columns = df_company.columns.str.rstrip('\n')
    df_company = df_company[df_company['State of Domicile'].isin(states)]
    #print(df_company)
    df_company_list = df_company['Name'].tolist()
    if display_company_select:
        company_selected = st.sidebar.selectbox('Company Options:', df_company_list)
        need_list_for_company = [company_selected]
        select_Company = 1
    elif company_compare:
        company1 = st.sidebar.selectbox('1st Company Option:', df_company_list)
        company2 = st.sidebar.selectbox('2nd Company Option:', df_company_list)
        company3 = st.sidebar.selectbox('3rd Company Option:', df_company_list)
        company4 = st.sidebar.selectbox('4th Company Option:', df_company_list)
        company_choice_list = [company1,company2,company3,company4]
        for company in company_choice_list:
            tickerOfName = df_company.loc[df_company['Name'] == company, 'Ticker']
            final_comp_ticker_list.append(tickerOfName)
        compare_company_select = 1
elif display_sector_select:
    sector_selected = st.sidebar.selectbox('Sector Options:', all_sectors)
    sector_list = [sector_selected]
    select_Sector = 1
...
for year in rebalancing.keys():
    ...
    df = df[df['State of Domicile'].isin(states)] #states included
    df = df.sort_values('Market Cap', ascending=False)
    if select_Sector == 1:
        df = df[df['GICS Sector'].isin(sector_list)] #sectors included
    if compare_company_select == 1:
        df = df[df['Name'].isin(company_choice_list)]  
    if select_Company == 1:
        df = df[df['Name'].isin(need_list_for_company)]  

    df = df[['Ticker','Equity Float', 'GICS Sector']].head(companies) #num of companies included
...
#if compare_company_select == 1: 
    #test_df == eod_market_cap_pivot[[final_comp_ticker_list[0],final_comp_ticker_list[1],final_comp_ticker_list[2],final_comp_ticker_list[3]]]
    #print(test_df)
...
#print(latest_date_merged_df['Market Cap'])
...
# Generate pie chart
if select_Sector == 1:
    sector_distribution = latest_date_merged_df[['Ticker','Market Cap']]
    fig_pie = px.pie(sector_distribution, names='Ticker', values='Market Cap', title=f'{sector_selected} Market Cap Distribution on {latest_date.strftime("%Y-%m-%d")}')
else: 
    sector_distribution = latest_date_merged_df.groupby('GICS Sector')['Market Cap'].sum().reset_index()
...
# Generate the treemap for sector distribution (company distribution if it is a specific sector)
if select_Sector == 1:
    fig_treemap = px.treemap(sector_distribution, path=['Ticker'], values='Market Cap',
                        title=f'{sector_selected} Market Cap Distribution on {latest_date.strftime("%Y-%m-%d")}',
                        color='Market Cap',
